-- =============================================================================
-- Factory ERP — Returns board: a written-off piece is closed, not active.
--
-- 0063 let QA write off a rejected piece the vendor is never going to return.
-- The board did not know about that state, so those pieces stayed in ACTIVE
-- returns — asking the order taker to complete a handback for a piece that has
-- been formally abandoned. Nothing they could do would ever clear it.
--
-- Body is 0060's verbatim; only the qa-rejection bucket expression changes.
-- =============================================================================
...
begin
  ...
  slots as (
    ...
    select d.sheet_id, 'damage'::text, d.id, d.created_at,
     ...
     where d.factory_id = v_factory
       ...
       -- 0059: a row that PASSED re-inspection produced a repeat (already in
       -- the union above) and a SUPERSEDED row handed its slot to a
       -- replacement. Counting either would number the sheet's pieces as
       -- though it physically had more than repeats_count of them.
  ...
  select
    ...
    rep.current_status,
    case
      when rep.current_status in ('awaiting_final_qa','awaiting_qa_final','completed') or rep.ot_return_confirmed_at is not null
        then 'completed'
      else 'active'
    end,
    st.stage_type,
  ...
  select
    ...
    'rejected_at_qa'::text,
    -- A written-off piece is CLOSED: nobody is going to hand it back, so it
    -- must not sit in Active returns asking the order taker to do exactly that.
    case
      when d.ot_return_confirmed_at is not null
        or coalesce(d.recheck_state, 'awaiting_return') = 'written_off'
  ...
  where d.factory_id = v_factory
    and d.stage_type = 'repeat_qa'
    and d.repeat_id is null
    -- Same filter, for the same reason: a piece that has since passed QA, or
    -- whose rejection was replaced by a fresh one, is not an entry on this
    -- board any more. Its successor (or its coded repeat) is.
    and coalesce(d.recheck_state,'awaiting_return')
          in ('awaiting_return','awaiting_recheck','written_off')
