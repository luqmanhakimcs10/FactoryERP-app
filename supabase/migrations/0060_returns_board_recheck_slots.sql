-- =============================================================================
-- Factory ERP — Returns board: exclude slots that the 0059 recheck loop closed.
--
-- 0059 gave a rejected piece a lifecycle instead of a single terminal state. Two
-- of those states mean "this damage row no longer represents a physical piece
-- waiting on anyone":
--   passed      — QA re-inspected and coded a repeat for the slot.
--   superseded  — QA rejected it again; a fresh row owns the slot now.
--
-- Without filtering them the board both (a) lists stale entries the Order Taker
-- has already dealt with, and (b) inflates every sheet's piece numbering,
-- because the slot CTE would count one physical piece two or three times over
-- as it bounced.
--
-- Body is 0058's verbatim plus those two filters.
-- =============================================================================
