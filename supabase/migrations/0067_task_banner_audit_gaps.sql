-- =============================================================================
-- Factory ERP — task banners, gaps found by auditing the app against itself.
--
-- WHY THIS MIGRATION EXISTS
-- -------------------------
-- 0065 defined the banner queues from a worked example list. That was the wrong
-- source: it produced banners for the queues someone happened to name, not for
-- the queues the app already counts. `scripts/audit-banner-coverage.mjs` puts
-- every count a role's SCREENS already display next to the banners its dashboard
-- renders, and three real, non-zero queues had no banner at all.
--
-- A count a screen is already willing to show is, by definition, a queue this
-- app considers worth counting. That is the right thing to audit against, and it
-- is what this migration closes.
--
-- FOUND BY THE AUDIT (Alpha):
--   Delivery    "Ready for final delivery"  9 orders  -> no banner
--   Procurement PO "To action" (executed)   1 PO      -> no banner
--   Floor Mgr   "Shift close" / "Leave"     0 today   -> no queue DEFINED at all
--
-- The two Floor Manager queues are zero in both factories right now, so nothing
-- is visibly broken today. They are defined anyway because the card that counts
-- them is already on the dashboard: the moment a leave request arrives, the old
-- behaviour would silently be the same gap again. A queue that returns zero
-- costs nothing — banners render only above zero.
--
-- WHAT IS DELIBERATELY *NOT* HERE
-- -------------------------------
-- QA's "Repeats & stage tracking" card shows 24. It is NOT given a banner: 24 is
-- how many orders are in production, not how many need QA. The actionable subset
-- is `qa_stage` (repeats at stage_qa), which is genuinely 0 right now. A banner
-- reading "24 pieces need stage QA" would send QA to an empty screen — worse
-- than no banner. Same reasoning for the Orders/Damages/Invoices cards, which
-- are registry totals, not work.
--
-- MODULE GUARDS ARE WHY THE PREDICATES ARE INLINED
-- ------------------------------------------------
-- The obvious implementation is `select count(*) from fm_shift_close_queue()`,
-- reusing the screen's own function. It is also a live bug: that function calls
-- assert_module('machine_workforce'), which RAISES for a factory with the module
-- off — and raising here would take out EVERY banner for that user, not just
-- this one. (Beta's floor manager already errors on that call.) So each queue
-- below inlines the predicate behind a boolean `module_enabled` check, which
-- returns false instead of throwing.
-- =============================================================================

-- ---------------------------------------------------------------------------
-- 1. Summary — extended with the audited queues. Body is 0065's, unchanged
--    except for the additions marked NEW.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 2. Items — the rows behind each new banner, each reusing the SAME predicate
--    as its count above so a banner saying 9 cannot open a list of 8.
-- ---------------------------------------------------------------------------
