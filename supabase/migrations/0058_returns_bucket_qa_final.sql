-- =============================================================================
-- Factory ERP — Returns board (superseded by 0060; kept for migration history).
--
-- The board buckets a repeat 'active' or 'completed' by enumerating statuses.
-- 0056 inserted `awaiting_qa_final` between the Floor Manager's final check and
-- QA's real final pass, so without this a repeat drops back to "active" the
-- moment the Floor Manager signs it off — and the Order Taker sees a piece they
-- have already returned reappear as outstanding.
--
-- Body is 0055's verbatim, with ONLY the bucket CASE changed. Restating the
-- whole function is deliberate: the alternative is a second definition of the
-- same query that has to be kept in step by hand.
-- =============================================================================
